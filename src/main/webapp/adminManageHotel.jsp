<%@page import="smartMukkam.com.hotel.HotelResetPassword"%>
  ...
  <ul class="nav justify-content-end">
		...
		<div class="container-sm">
			...
			<div style="height: 400px; width: auto;  overflow: auto;" id="contentToRefreshTwo">
			    ...
			    <tbody>
					<%
					List<HotelResetPassword> ChangePass = ServicesDAO.getHotelRequestChangePass();
					for(HotelResetPassword pass : ChangePass){
					%>
					<tr>
						<td><img src="himg?id=<%=pass.getHoid()%>" style="width: 40px; border: 1px solid; border-radius: 5px;" alt="" ></td>
					    <td><%=HotelDAO.getHotelName(pass.getHoid()) %></td>
					    <td><%=pass.getEmail() %></td>
					    <td><%=pass.getPass() %></td>
					    <td>
					    			<div class="dropdown">
				                ...
				                %>;"
					              >
					                ...
					                <ul class="dropdown-menu">
					                    <li><a href="#" class="dropdown-item" onclick="sendEmail('<%= pass.getEmail() %>','<%= pass.getPass()%>','<%=HotelDAO.getHotelName(pass.getHoid())%>')">Send email</a></li>
		...
		<div class="container-sm">
			...
			<div style="height: 400px; width: auto; overflow: auto;">
			    ...
			    <%
				  ...
				  %>
				    <tr>
				      <th scope="row"><img src="himg?id=<%=r.getHoid()%>" style="width: 60px; border: 1px solid; border-radius: 5px;" alt="" ></th>
				      <th scope="row"><img src="imRoom?id=<%=r.getTid()%>" style="width: 60px; border: 1px solid; border-radius: 5px;" alt="" ></th>
				      <td><%=r.getRoom() %></td>
				      <td><%=r.getNoRoom() %></td>
		...
		<div class="container-sm">
			...
			<div style="height: 400px; width: auto; overflow: auto;">
			    ...
			    %>
			    <tr>
			    	<th scope="row"><img src="himg?id=<%=f.getHoid()%>" style="width: 60px; border: 1px solid; border-radius: 5px;" alt="" ></th>
			    	<th scope="row"><img src="imfood?id=<%=f.getTid()%>" style="width: 60px; border: 1px solid; border-radius: 5px;" alt="" ></th>
				      <td><%=f.getFood() %></td>
				      <td><%=f.getQuandity() %></td>
		...
		<div class="container-sm">
			...
			<div style="height: 400px; width: auto; overflow: auto;">
			    ...
			    <tbody>
				  ...
				  %>
				    <tr>
				    	<th scope="row"><img src="himg?id=<%=t.getHoid()%>" style="width: 60px; border: 1px solid; border-radius: 5px;" alt="" ></th>
				    	<th scope="row"><img src="imTaxi?id=<%=t.getTid()%>" style="width: 60px; border: 1px solid; border-radius: 5px;" alt="" ></th>
				    	<td><%=t.getVehicle() %></td>
				    	<td><%=t.getDriver() %></td>
				    	<td><%=t.getPlate() %></td>
				    	<td><%=t.getPrice() %></td>
...
$(document).ready(function() {
    ...
    function bindEventHandlersTwo() {
        ...
        $(".acceptLinkPassword").on("click", function(event) {
            ...
            $.ajax({
                type: "GET",
                url: "hotelResetPasswordAccept.jsp",
                data: { id: appointmentId },
                success: function(response) {
                    // Handle the success response (if needed)
                    console.log("Registration accepted successfully");
        ...
        $(".rejectLinkPassword").on("click", function(event) {
            ...
            $.ajax({
                type: "GET",
                url: "hotelResetPasswordReject.jsp",
                data: { id: appointmentId },
                success: function(response) {
                    // Handle the success response (if needed)
