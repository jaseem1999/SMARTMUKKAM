    ...
    <div class="col-sm-10" >
    	<div class="homeFull" style="margin: 50px; max-height: 300px; overflow: auto;"  >
			...
			<table class="table table-striped table-hover" id="contentToRefresh">
  					<thead>
					    <tr>
					      <th scope="col">User and Verification</th>
					  ...
					  <tbody>
					  <%
					  List<ReviewDTO> review = ShopDAO.getReviewBasedSid(sid);
					  for(ReviewDTO r : review){
						 
					  %>
						<tr>
							<td><img alt="" style="width: 40px; border: 1px solid; border-radius: 50%" src="userPhoto?id=<%=r.getUid()%>">
							<%
							...
							<td><%=ShopDAO.getProduct(r.getPid()) %></td>
							<td><img alt="" src="pImage?id=<%=r.getPid()%>" style="width: 75px; height: 75px;"></td>
							<td><%=r.getReview() %></td>
							<td>
...
$(document).ready(function () {
    ...
    $('#contentToRefresh').on('click', '.submitReview', function () {
        ...
        $.ajax({
            ...
            url: 'shopReviewComment.jsp', // replace with the actual server endpoint
            data: formData,
